...
export const PERFORMANCE_DATA: ChartData[] = Array.from({ length: 50 }).map((_, i) => {
  // Simulate logarithmic growth curves
  const x = i + 1;
  return {
    iteration: x,
    staticLLM: 20 + 15 * Math.log10(x), // Slower growth
    gollum: 20 + 35 * Math.log10(x) + (Math.random() * 2) // Faster growth, higher cap
  };
});
...
export const CONTENT = {
  ...
  RESULTS: {
    ...
    In benchmarks like the Buchwald-Hartwig reactions, this approach nearly doubles the discovery rate of high-yielding conditions compared to static embeddings or traditional descriptors.
